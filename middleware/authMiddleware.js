...
// ==========================================
// 1. Verificación del Token (Priorizando Headers)
// ==========================================
export const verifyToken = async (req, res, next) => {
  try {
    let token = null;

    // 🚩 1. PRIMERO buscamos en los Headers (El estándar Bearer)
    if (
    ...
    // 🚩 2. Si no hay en Header, intentamos leer la vieja cookie como fallback
    ...
    if (!token || token === 'null' || token === 'undefined') {
      logger.warn(
        'Intento de acceso rechazado: No se detectó Header de autorización Bearer.',
      );
    ...
    // 🚩 El error 401 aquí es vital para que el Frontend sepa que debe usar el Refresh Token
    if (error.name === 'TokenExpiredError') {
      return res.status(401).json({
        success: false,
        message: 'El tiempo de su sesión ha concluido. Inicie sesión de nuevo.',
      });
    }
    return res.status(401).json({
      success: false,
